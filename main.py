...
async def api_import_bl(request: Request, file: UploadFile=File(...), db=Depends(get_db)):
    ...
    for i,row in enumerate(reader,2):
        try:
            # Accept both French and English column names
            yr   = int(row.get("YEAR") or row.get("ANNEE") or row.get("year") or 0)
            ...
            raw_bcp = str(row.get("BUDGET CP (FCFA)") or row.get("BUDGET CP") or row.get("budget_cp",0) or "0")
            bcp  = float(raw_bcp.replace(" ","").replace(" ","").replace(" ","").replace(",",".").replace(" ",""))
            if not yr or not dirn or not imp: errors.append(f"Ligne {i}: données manquantes"); continue
            ex = db.execute("SELECT id FROM budget_lines WHERE year=? AND direction=? AND imputation=?",(yr,dirn,imp)).fetchone()
            if ex:
                db.execute("UPDATE budget_lines SET libelle=?,nature=?,budget_cp=? WHERE id=?",(lib,nat,bcp,ex["id"])); updated+=1
            else:
