...
def api_me(request: Request, db=Depends(get_db)):
    u = require_login(request)
    # Return years that actually have budget lines data
    years_raw = db.execute("SELECT DISTINCT year FROM budget_lines ORDER BY year DESC").fetchall()
    years = [r["year"] for r in years_raw] if years_raw else [2025]
    tx_years_raw = db.execute("SELECT DISTINCT year FROM transactions ORDER BY year DESC").fetchall()
    tx_years = [r["year"] for r in tx_years_raw]
    all_years = sorted(set(years + tx_years), reverse=True) or [2025]
    return {**u, "direction_list": user_dirs(u), "available_years": all_years, "default_year": all_years[0]}
...
async def upload_attachment(request: Request, tx_id: int, file: UploadFile=File(...), db=Depends(get_db)):
    u = require_login(request)
    import base64, json as _json
    raw = await file.read()
    if len(raw) > 5*1024*1024: raise HTTPException(400,"Fichier trop grand (max 5MB)")
    ...
    try: atts = _json.loads(tx["attachments"] or "[]")
    except: atts = []
    atts.append({"name": file.filename, "mime": mime, "data": b64, "uploaded_by": u.get("name",""), "uploaded_at": date.today().isoformat()})
    db.execute("UPDATE transactions SET attachments=? WHERE id=?",(str(_json.dumps(atts)),tx_id))
    db.commit()
    return {"ok": True, "count": len(atts), "name": file.filename}

@app.delete("/api/transactions/{tx_id}/attachments/{att_idx}")
def delete_attachment(request: Request, tx_id: int, att_idx: int, db=Depends(get_db)):
    ...
    if 0 <= att_idx < len(atts): atts.pop(att_idx)
    db.execute("UPDATE transactions SET attachments=? WHERE id=?",(str(_json.dumps(atts)),tx_id))
    db.commit()
    return {"ok": True, "count": len(atts)}

@app.get("/api/transactions/{tx_id}/attachments")
def get_attachments(request: Request, tx_id: int, db=Depends(get_db)):
    import json as _json
    require_login(request)
    tx = db.execute("SELECT attachments FROM transactions WHERE id=?",(tx_id,)).fetchone()
    if not tx: raise HTTPException(404,"Not found")
    try: atts = _json.loads(tx["attachments"] or "[]")
    except: atts = []
    # Return metadata only (no base64 data)
    return [{"index": i, "name": a["name"], "mime": a["mime"], "uploaded_by": a.get("uploaded_by",""), "uploaded_at": a.get("uploaded_at","")} for i,a in enumerate(atts)]

@app.get("/api/transactions/{tx_id}/attachments/{att_idx}/download")
def download_attachment(request: Request, tx_id: int, att_idx: int, db=Depends(get_db)):
    ...
    if att_idx < 0 or att_idx >= len(atts): raise HTTPException(404,"Attachment not found")
    att = atts[att_idx]
    data = _b64.b64decode(att["data"])
    return StreamingResponse(iter([data]), media_type=att["mime"], headers={"Content-Disposition": f'attachment; filename="{att["name"]}"'})
...
/* Watermark */
.wm{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-25deg);font-size:160pt;font-weight:900;color:rgba(0,176,232,0.06);pointer-events:none;z-index:0;font-family:Arial Black,sans-serif;user-select:none;}
...
.sign-only{border:1.5px solid #334155;padding:2mm 3mm;min-height:15mm;}
.sign-only label{font-size:8pt;font-weight:900;text-transform:uppercase;color:#003e7e;display:block;margin-bottom:2mm;letter-spacing:.08em;}
.sign-line{border-bottom:1px solid #334155;margin:3mm 0 1.5mm;}
.sign-name{font-size:7.5pt;color:#475569;}
.sign-date{font-size:7.5pt;color:#475569;margin-top:1mm;}
@media print{
...
<div class='fiche'>
  ...
  <div class='fhdr'>
    <div class='flogo'>
      <svg width='36' height='36' viewBox='0 0 100 100' fill='none'>
        ...
        <path d='M72 28 L54 28 C34 28 20 37 20 50 C20 63 34 72 54 72 L72 72 L72 59 L56 59 C45 59 37 55 37 50 C37 45 45 41 56 41 L72 41 Z' fill='white'/>
        <path d='M72 28 L91 50 L72 72 Z' fill='white' opacity='0.8'/>
      </svg>
    </div>
    <div class='ftitle'>
  ...
  <div class='signs'>
    <div class='sign sign-only'>
      <label>BUDGET SERVICE / SAAF</label>
      <div class='sign-line'></div>
      <div class='sign-name'>Name / Nom : _________________________</div>
